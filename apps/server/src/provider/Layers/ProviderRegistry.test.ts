    ...
    describe("checkCodexProviderStatus", () => {
      ...
      it.effect("returns unavailable when codex is below the minimum supported version", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            status.message,
            "Codex CLI v0.36.0 is too old for T3Nexus. Upgrade to v0.37.0 or newer and restart T3Nexus.",
          );
        }).pipe(
          Effect.provide(
            mockSpawnerLayer((args) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in T3Nexus settings.");
        }),
      );
    });
